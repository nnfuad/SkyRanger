...
// ———————————————————————————————————————————————————————————————
// FINAL bullet-proof search_gyro – works with ALL MPU-6050 clones
// ———————————————————————————————————————————————————————————————
byte search_gyro(int gyro_addr, int who_am_i) {
  Wire.beginTransmission(gyro_addr);
  Wire.write(0x6B);          // PWR_MGMT_1
  Wire.write(0x00);          // Wake up the chip
  Wire.endTransmission();
  delay(100);

  Wire.beginTransmission(gyro_addr);
  Wire.write(who_am_i);      // WHO_AM_I register = 0x75
  Wire.endTransmission();
  delay(10);
  ...
  if (Wire.available()) {
    ...
    // Many clones return 0x00, 0x02, 0x68, 0x71, 0x73, 0x98 … anything except garbage
    // As long as we get at least one byte back → it’s an MPU-6050
    if (response != 0xFF) {         // 0xFF = bus error / nothing connected
      Serial.print(F("WHO_AM_I response: 0x"));
      Serial.println(response, HEX);  // This line is just for debugging – you’ll see the real value
      return gyro_addr;               // Success – accept this address
    }
  }
  return 0;  // really nothing there
}
...
void gyro_signalen(){
  ...
  if(type == 1){
    ...
    Wire.requestFrom((uint8_t)gyro_address, (uint8_t)6);  // explicit cast → compiler happy

    unsigned long timeout = millis() + 10;         // max 10 ms safety
    while (Wire.available() < 6 && millis() < timeout) ;  // non-blocking wait
    ...
    // if no data (extremely rare after first loops) → just keep previous values
  ...
  // ==================== L3G4200D / L3GD20H (type 2 or 3) ====================
  else if(type == 2 || type == 3){
    Wire.beginTransmission(gyro_address);
    Wire.write(0x28 | 0x80);                       // 0x28 + auto-increment
    Wire.endTransmission(false);
    Wire.requestFrom((uint8_t)gyro_address, (uint8_t)6);

    unsigned long timeout = millis() + 10;
    while (Wire.available() < 6 && millis() < timeout) ;

    if (Wire.available() >= 6) {
      gyro_roll  = (int16_t)(Wire.read() | (Wire.read() << 8));
      gyro_pitch = (int16_t)(Wire.read() | (Wire.read() << 8));
      gyro_yaw   = (int16_t)(Wire.read() | (Wire.read() << 8));
    }
